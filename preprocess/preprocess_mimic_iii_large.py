...
# mimic_data_dir = '/home/covpreduser/Blob/shunzhen/ClinicalTherapeuticsModeling/mimic-iii-clinical-database-1.4/'
...
# new_adm = adm.loc[((adm.DISCHTIME-adm.ADMITTIME)>=pd.Timedelta(48,'h'))]
# new_adm = new_adm.loc[~(new_adm.HADM_ID.isna())]
...
# null_p = [[] for _ in range(len(feature_set))]

# for _,p_row in tqdm(new_adm.iterrows(), total=new_adm.shape[0]):
#     tmp_adm = p_row.HADM_ID
#     adm_id.append(tmp_adm)
#     label.append(int(p_row.HOSPITAL_EXPIRE_FLAG))

#     patient = null_p
#     p = events.loc[events.HADM_ID.isin([tmp_adm])]
...
# pickle.dump([adm_id, data, label], open('/home/covpreduser/Blob/v-jiawezhang/ClinTS_HII/preprocess/patient_records_large.p','wb'))
